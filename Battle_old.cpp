...
private:
  ...
  void displayHealth(const Pokemon &pokemon) {
    double healthPercentage =
        (static_cast<double>(pokemon.current_hp) / pokemon.hp) * 100;
    std::cout << "\n" + pokemon.name << " HP: " << healthPercentage << "%"
              << std::endl;
  }
...
public:
  ...
  void selectOpponentPokemon() {
    ...
    int randomIndex =
        rand() % opponentTeam.PokemonTeam.size(); // Get random index
    opponentSelectedPokemon =
        opponentTeam.PokemonTeam.at(randomIndex); // Select opponent Pokémon
    ...
    while (true) {
      ...
      while (true) {
        ...
        if (chosenMoveIndex >= 1 &&
            chosenMoveIndex <= selectedPokemon.moves.size()) {
          break; // Valid choice
        }
      ...
      //         cout << "\n" << selectedPokemon.name << " has fainted!" <<
      //         endl; displayHealth(selectedPokemon);

      //         // Select a new Pokémon if available
      //         bool foundNewPokemon = false;
      //         for (auto& pair : playerTeam.PokemonTeam) {
      //             if (pair.second.current_hp > 0) {  // Find first
      //             non-fainted Pokémon
      //                 selectedPokemon = pair.second;
      //                 cout << "\nYou send out " << selectedPokemon.name <<
      //                 "!\n"; foundNewPokemon = true; break;
      //             }
      //         }

      //         if (!foundNewPokemon) {
      //             cout << "\nAll your Pokémon have fainted! You lost the
      ...
      //             cout << "\nAll opponent’s Pokémon have fainted! You won the
      //             battle!\n"; return;  // End the battle
      //         }

      //         // Show remaining Pokémon count
      //         cout << "Opponent has " << remainingPokemon << " Pokémon
      //         left.\n";

      //         // Seed random number generator to avoid the same selection
      //         int randomIndex = rand() % availableIndices.size();
      //         opponentSelectedPokemon =
      //         opponentTeam.PokemonTeam[availableIndices[randomIndex]];
