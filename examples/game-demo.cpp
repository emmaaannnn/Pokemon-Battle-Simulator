...
int main() {
  ...
  // Team data
  const auto teams = std::unordered_map<std::string, std::vector<std::string>>{
      {"Team 1", {"venusaur", "pikachu", "machamp"}},
      {"Team 2", {"charizard"}},
      {"Brock Team", {"golem", "onix"}}};

  const auto moves = std::unordered_map<
      std::string,
  ...
  const auto chosenTeam = "Team " + std::to_string(teamChoice);
  ...
  const auto *chosenPokemon = playerTeam.getPokemon(pokemonChoice - 1);
  const auto *opponentPokemon =
  ...
  if (chosenPokemon && opponentPokemon) {
    ...
    for (size_t i = 0; i < chosenPokemon->moves.size(); ++i) {
      const auto &move = chosenPokemon->moves[i];
      std::cout << "[" << (i + 1) << "] " << move.name;
      if (move.power > 0) {
